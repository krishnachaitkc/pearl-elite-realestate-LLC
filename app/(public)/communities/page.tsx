...
export const metadata: Metadata = {
  title: "Communities",
  description: "Explore premium communities and neighbourhoods across Dubai and the UAE.",
};
    ...
    <div className="min-h-screen bg-[var(--color-charcoal-900)] pt-20">
      <div className="bg-[var(--color-charcoal-900)] py-14">
        <div className="container-site">
          <span className="text-label text-[var(--color-brand-400)] block mb-2">Neighbourhoods</span>
          <h1 className="heading-section text-white text-3xl lg:text-4xl">Premium Communities</h1>
          <p className="text-[var(--color-charcoal-400)] mt-3 max-w-lg">
            Discover Dubai&apos;s most sought-after communities and find your perfect neighbourhood.
          </p>
        </div>
      </div>
      ...
      <div className="container-site py-12">
        {communities.length === 0 ? (
          <div className="text-center py-20 bg-[var(--color-charcoal-800)] rounded-2xl">
            <p className="text-[var(--color-charcoal-400)]">No communities listed yet.</p>
          </div>
        ) : (
            ...
            {communities.map((community, index) => (
              ...
              >
                <div className="relative h-52 overflow-hidden">
                  <img
                    src={community.imageUrl || fallbackImages[index % fallbackImages.length]}
                    alt={community.name}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    {community.location && (
                      <div className="flex items-center gap-1 text-xs text-white/70 mb-1">
                        <MapPin size={10} /><span>{community.location.name}</span>
                      </div>
                    )}
                    ...
                    <p className="text-xs text-white/60 mt-0.5">
                      {community._count.properties} Properties
                    </p>
                  </div>
                </div>
                {community.description && (
                  <div className="p-4">
                    <p className="text-xs text-[var(--color-charcoal-400)] line-clamp-2">{community.description}</p>
                    <div className="flex items-center gap-1 text-xs text-[var(--color-brand-500)] mt-3 font-medium">
                      Explore Community <ArrowRight size={11} />
                    </div>
                  </div>
                )}
              </Link>
