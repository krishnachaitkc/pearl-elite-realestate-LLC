...
*/
const CONFIG_INIT_SERVICES = NEON_SERVICES.filter((service) => service !== "postgres" && service !== "data-api");
/**
...
*/
const CONFIG_INIT_NONE_MEANS = "the bare starter policy";
/** Why the two a policy cannot declare are not selectable, for the refusal message. */
const CONFIG_INIT_UNAVAILABLE = {
...
const FUNCTION_SLUG = "hello";
const FUNCTION_NAME = "Hello World";
const FUNCTION_FILENAME = "hello.ts";
...
const renderPreview = (services) => {
	const lines = [];
	if (services.includes("ai-gateway")) lines.push(...at(2, "aiGateway: true,"));
	if (services.includes("functions")) lines.push(...block(2, "functions", [...at(3, `${FUNCTION_SLUG}: { name: "${FUNCTION_NAME}", source: "./${FUNCTION_FILENAME}" },`)]));
	if (services.includes("object-storage")) lines.push(...block(2, "buckets", [...at(3, `// "private" is the default; use "public_read" for anonymous reads`, `${BUCKET_NAME}: { access: "private" },`)]));
...
*/
const renderNeonConfig = (services) => `import { defineConfig } from "${CONFIG_PACKAGE}/v1";
...
const renderSeededBranch = (view) => {
	const settings = [];
	if (view.branch?.parent !== void 0) settings.push(...at(2, `parent: ${renderScalar(view.branch.parent)},`));
	if (view.branch?.ttl !== void 0) settings.push(...at(2, `ttl: ${renderScalar(view.branch.ttl)},`));
	const compute = view.branch?.postgres?.computeSettings;
	const computeFields = Object.entries(compute ?? {}).filter(([, value]) => value !== void 0);
	if (computeFields.length > 0) settings.push(...block(2, "postgres", [...block(3, "computeSettings", computeFields.flatMap(([field, value]) => at(4, `${field}: ${renderScalar(value)},`)))]));
...
const renderSeededPreview = (view, branchName) => {
	...
	if (buckets.length > 0) lines.push(...block(2, "buckets", buckets.flatMap(([name, bucket]) => at(3, `${renderKey(name)}: { access: "${bucket.access}" },`))));
	const functions = Object.entries(view.preview?.functions ?? {});
	if (functions.length > 0) lines.push(...at(2, `// ${branchName} has ${functions.length} deployed function${functions.length === 1 ? "" : "s"}.`, "// Declaring one needs the local source path, which the branch does not know:", "// functions: {", ...functions.map(([slug, fn]) => `//   ${renderKey(slug)}: { name: "${fn.name}", source: "./${slug}.ts" },`), "// },"));
	if (lines.length === 0) return "";
	return `${block(1, "preview", lines).join("\n")}\n`;
};
/**
...
*/
const renderNeonConfigFromView = (view, branchName) => {
	const services = [view.auth ? "  auth: true," : "", view.dataApi ? "  dataApi: true," : ""].filter((line) => line !== "");
	const preview = renderSeededPreview(view, branchName);
	const branch = renderSeededBranch(view);
	if (services.length === 0 && preview === "" && branch === "") return {
		source: renderNeonConfig([]),
		seeded: false
	};
	const protectedNote = view.branch?.protected ? `// ${branchName} is protected on Neon. Not declared here: a policy \`protected\` would\n// apply to every branch this policy is applied to.\n` : "";
	const body = [
		...services,
...
*/
const FUNCTION_TEMPLATE = `export default async function hello(): Promise<Response> {
  return new Response("Hello from Neon Functions");
}
`;
//#endregion
export { BUCKET_NAME, CONFIG_INIT_NONE_MEANS, CONFIG_INIT_SERVICES, CONFIG_INIT_UNAVAILABLE, CONFIG_PACKAGE, ENV_PACKAGE, FUNCTION_FILENAME, FUNCTION_NAME, FUNCTION_SLUG, FUNCTION_TEMPLATE, REQUIRED_PACKAGES, renderNeonConfig, renderNeonConfigFromView };
