...
const single = (name, { required = false } = {}) => (value) => {
	...
	if (Array.isArray(value)) throw new Error(`--${name} was given more than once. Pass it at most once.`);
	if (value === false) throw new Error(required ? `--no-${name} is not valid: --${name} is required.` : `--no-${name} is not a valid way to skip --${name}. Omit the flag entirely.`);
	if (typeof value !== "string" || value.trim() === "") throw new Error(required ? `--${name} needs a value.` : `--${name} needs a value. Pass one, or omit the flag entirely.`);
	return value;
};
/**
...
*/
const noPassthrough = (command) => (argv) => {
	const rest = argv["--"];
	if (Array.isArray(rest) && rest.length > 0) throw new Error(`${command} takes no arguments after \`--\`, and options placed there are ignored rather than applied. Remove the \`--\`.`);
	return true;
};
//#endregion
export { noPassthrough, single };
