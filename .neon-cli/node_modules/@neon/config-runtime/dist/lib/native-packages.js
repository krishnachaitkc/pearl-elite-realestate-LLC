import { installedPackageVersion, readPackageManifest } from "./resolve-package.js";
import { ErrorCode, PlatformError, externalPackageRoot } from "@neon/config";
import { join, resolve, sep } from "node:path";
import { spawn } from "node:child_process";
import { lstatSync, mkdtempSync, readFileSync, rmSync, writeFileSync } from "node:fs";
import { tmpdir } from "node:os";
//#region src/lib/native-packages.ts
/**
...
const RUNTIME_TARGET_LABEL = `${RUNTIME_TARGET.os}-${RUNTIME_TARGET.cpu} (${RUNTIME_TARGET.libc})`;
/**
...
*/
function isIncompatibleWithRuntime(packageDir) {
	const manifest = readPackageManifest(packageDir);
	return manifest !== void 0 && !manifestRunsOnRuntime(manifest);
}
/** Whether a package name reads as a WebAssembly build. */
function isWasmVariantName(packageName) {
...
*/
function platformFamily(packageName) {
	const [scope, rest] = packageName.startsWith("@") ? [`${packageName.split("/")[0]}/`, packageName.split("/")[1] ?? ""] : ["", packageName];
...
*/
const PLATFORM_TOKENS = /* @__PURE__ */ new Set([
	"wasm32",
	"linux",
	"linuxmusl",
	"darwin",
...
*/
function concreteUnresolved(unresolved) {
	return [...new Set(unresolved.filter((specifier) => specifier.startsWith("./") || specifier.startsWith("../")))];
}
/** Whether a manifest's `os`/`cpu`/`libc` permit {@link RUNTIME_TARGET}. */
function manifestRunsOnRuntime(manifest) {
	const permits = (raw, value) => {
		const field = typeof raw === "string" ? [raw] : Array.isArray(raw) ? raw.filter((entry) => typeof entry === "string") : void 0;
...
*/
function packageOfArchivePath(path) {
	...
	if (scoped && parts.length < 2) return void 0;
	return {
		name: parts.join("/"),
...
const invalid = (message, cause) => new PlatformError(ErrorCode.InvalidConfig, message, cause === void 0 ? void 0 : { cause });
/**
...
*/
async function traceNativePackages(options) {
	...
	const specs = roots.map((pkg) => {
		...
		if (version === void 0) throw invalid(`Function "${slug}" stages "${pkg}", but no installed copy of it could be found in this project, so the deploy cannot tell which version you tested against. Install "${pkg}" as a dependency, or exclude it if the function never reaches it:\n    externalPackages: [{ name: "${pkg}", includeFiles: false }]`);
		return `${pkg}@${version}`;
	});
	const staging = mkdtempSync(join(tmpdir(), "neon-fn-native-"));
	try {
		writeFileSync(join(staging, "package.json"), `${JSON.stringify({
			name: "neon-fn-native-staging",
			private: true
		}, null, 2)}\n`);
		await deps.install(staging, specs);
		verifyInstalled(slug, roots, staging);
		writeFileSync(join(staging, "trace-entry.mjs"), packages.map((pkg) => `import ${JSON.stringify(pkg)};`).join("\n"));
		const traced = await deps.trace(staging, "trace-entry.mjs");
		for (const specifier of concreteUnresolved(traced.unresolved ?? [])) warnings.push(`Function "${slug}" stages a package that imports "${specifier}", which is not in the tree the deploy installed. The import is relative to a package that *was* installed, so it will not appear in the archive and nothing else will notice. If the package reaches it, the function fails at invoke with "Cannot find module". If the import is wrapped in a try/catch — a native accelerator behind a JavaScript fallback — the deploy is already correct.`);
		const entries = readArchiveFiles(slug, staging, selectArchiveFiles(slug, traced.files, staging));
...
*/
function verifyInstalled(slug, packages, staging) {
	for (const pkg of packages) {
		const manifest = join(staging, "node_modules", pkg, "package.json");
		if (lstatSync(manifest, { throwIfNoEntry: false }) === void 0) throw invalid(`Function "${slug}" declares "${pkg}" in externalPackages, but it did not install for the Functions runtime (${RUNTIME_TARGET_LABEL}). Either the package does not publish a build for that platform, or it compiles from source at install time, which the deploy cannot do. Check that it lists a ${RUNTIME_TARGET.os}-${RUNTIME_TARGET.cpu} build among its optional dependencies. If this function never reaches "${pkg}", exclude it instead:\n    externalPackages: [{ name: "${pkg}", includeFiles: false }]`);
	}
}
/**
...
*/
function selectArchiveFiles(slug, traced, staging) {
	const inTree = traced.filter((path) => path.startsWith(`node_modules${sep}`) || path.startsWith("node_modules/")).map((path) => path.split(sep).join("/"));
	const incompatible = /* @__PURE__ */ new Map();
	const isIncompatible = (dir) => {
		const known = incompatible.get(dir);
		if (known !== void 0) return known;
		const decision = isIncompatibleWithRuntime(join(staging, ...dir.split("/")));
		incompatible.set(dir, decision);
		return decision;
	};
	const compatible = inTree.filter((path) => {
		const pkg = packageOfArchivePath(path);
		return pkg !== void 0 && !isIncompatible(pkg.dir);
	});
	/**
	...
	*/
	const variantKey = (name, dir) => {
		const version = readPackageManifest(join(staging, ...dir.split("/")))?.version;
		return `${platformFamily(name)}@${typeof version === "string" ? version : "?"}`;
	};
	const nativeBuilds = new Set(compatible.filter((path) => BINARY_PATTERN.test(path)).flatMap((path) => {
		const pkg = packageOfArchivePath(path);
		return pkg === void 0 ? [] : [variantKey(pkg.name, pkg.dir)];
	}));
	const files = compatible.filter((path) => {
		const pkg = packageOfArchivePath(path);
		if (pkg === void 0) return true;
		return !(isWasmVariantName(pkg.name) && nativeBuilds.has(variantKey(pkg.name, pkg.dir)));
	}).sort();
	if (files.length === 0) throw invalid(`Function "${slug}" stages external packages, but tracing the installed packages found no files to ship. This is a bug in the deploy — please report it at https://github.com/neondatabase/neon-pkgs/issues.`);
	return files;
}
/**
...
*/
function readArchiveFiles(slug, staging, files) {
	const entries = {};
	for (const relative of files) {
		const absolute = resolve(staging, relative);
		if (!absolute.startsWith(`${resolve(staging)}${sep}`)) throw invalid(`Function "${slug}": traced file "${relative}" resolves outside the staging directory. This is a bug in the deploy — please report it.`);
		const stat = lstatSync(absolute, { throwIfNoEntry: false });
		if (stat === void 0) throw invalid(`Function "${slug}": traced file "${relative}" disappeared before it could be read. This is a bug in the deploy — please report it at https://github.com/neondatabase/neon-pkgs/issues.`);
		if (stat.isSymbolicLink()) throw invalid(`Function "${slug}" would ship a symbolic link, which the build does not support: ${relative}. This is a bug in the deploy — please report it at https://github.com/neondatabase/neon-pkgs/issues.`);
		if (!stat.isFile()) continue;
		const contents = readFileSync(absolute);
		if (BINARY_PATTERN.test(relative)) verifyArchitecture(slug, relative, contents);
...
*/
function verifyArchitecture(slug, relative, contents) {
	if (contents.length < ELF.headerBytes) throw invalid(`Function "${slug}" would ship "${relative}", which is ${contents.length} bytes — too short to be a valid binary. The package may have been installed incompletely; reinstall it and try again.`);
	if (!(contents.subarray(0, 4).toString("binary") === ELF.magic)) throw invalid(`Function "${slug}" would ship "${relative}", which is not a Linux binary. The Functions runtime is ${RUNTIME_TARGET_LABEL}, so a binary built for another platform cannot load there. A package that compiles from source at install time produces a binary for the machine that installed it and cannot be cross-installed by the deploy.`);
	const machine = contents.readUInt16LE(ELF.machineOffset);
	if (machine !== ELF.aarch64) throw invalid(`Function "${slug}" would ship "${relative}", which is built for a different architecture (ELF machine 0x${machine.toString(16)}). The Functions runtime is ${RUNTIME_TARGET_LABEL}.`);
}
/**
...
*/
function enforceLimits(slug, entries, limits = DEFAULT_ARCHIVE_LIMITS) {
	const names = Object.keys(entries);
	const count = names.length;
	if (count > limits.maxEntries) throw invalid(`Function "${slug}" archive has ${count} files; the limit is ${limits.maxEntries}. Native packages bring their whole file tree — ship fewer of them, or split the work across functions.`);
	const total = names.reduce((sum, name) => sum + entries[name].length, 0);
	if (total > limits.maxTotalBytes) throw invalid(`Function "${slug}" archive is ${bytes(total)} uncompressed; the limit is ${bytes(limits.maxTotalBytes)}.\n\n${describeLargest(entries)}`);
}
/**
...
* finished ZIP.
*/
function assertZipWithinLimits(slug, zip, entries, limits = DEFAULT_ARCHIVE_LIMITS) {
	if (zip.length <= limits.maxZipBytes) return;
	throw invalid(`Function "${slug}" archive is ${bytes(zip.length)} compressed; the limit is ${bytes(limits.maxZipBytes)}.\n\n${describeLargest(entries)}`);
}
/** The handful of files worth naming when an archive is too big. */
function describeLargest(entries) {
	return `Largest contributors:\n${Object.entries(entries).sort(([, a], [, b]) => b.length - a.length).slice(0, 4).map(([name, data]) => `  ${bytes(data.length).padStart(9)}  ${name}`).join("\n")}\n\nNative binaries are large. Ship fewer of them, or split the work across functions.`;
}
const defaultDeps = {
	...
	trace: async (base, entry) => {
		...
		return {
			...
			unresolved: [...result.warnings].flatMap((warning) => {
				...
				return specifier === void 0 ? [] : [specifier];
			})
		};
	},
	installedVersion: (projectDir, pkg) => installedPackageVersion(projectDir, pkg)
};
/**
...
*/
async function runNpmInstall(cwd, specs) {
	const args = [
		"install",
		...specs,
	...
	const { code, stderr, spawnError } = await new Promise((resolveRun) => {
		...
		child.on("error", (spawnError) => resolveRun({
			code: null,
			stderr,
			spawnError
		}));
	...
	if (spawnError !== void 0) {
		...
		throw invalid(missing ? `Shipping a function's externalPackages requires "npm" on PATH, to install them for the Functions runtime (${RUNTIME_TARGET_LABEL}). It could not be found. Install Node.js with npm, or set includeFiles: false on every entry so nothing needs staging.` : `Failed to install externalPackages for the Functions runtime: ${spawnError.message}`, spawnError);
	}
	if (code !== 0) {
		if (stderr.includes("EBADPLATFORM")) throw invalid(`One of ${specs.map((s) => `"${s}"`).join(", ")} has no build for the Functions runtime (${RUNTIME_TARGET_LABEL}), so its files cannot be staged. Use a package that publishes a ${RUNTIME_TARGET.os}-${RUNTIME_TARGET.cpu} build (sharp does). If this function never reaches it, exclude it instead:\n    externalPackages: [{ name: "<package>", includeFiles: false }]\n\nnpm reported:\n${stderr.trim()}`);
		throw invalid(`Failed to install externalPackages for the Functions runtime (${RUNTIME_TARGET_LABEL}). npm exited with code ${code}.\n${stderr.trim()}`);
	}
}
/**
...
async function loadNft() {
	try {
		...
		throw invalid("Staging a function's external packages requires `@vercel/nft`, which could not be loaded. Installed from npm it is a dependency of @neon/config-runtime, so reinstalling your dependencies fixes it. If you are running the standalone `neon` binary, this is a packaging bug — please report it at https://github.com/neondatabase/neon-pkgs/issues.", cause);
	}
}
//#endregion
export { DEFAULT_ARCHIVE_LIMITS, RUNTIME_TARGET, RUNTIME_TARGET_LABEL, assertZipWithinLimits, enforceLimits, traceNativePackages };

//# sourceMappingURL=native-packages.js.map