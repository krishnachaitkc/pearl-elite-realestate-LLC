...
const setHelpPagerOpener = (opener) => {
...
* straight to `out`.
*/
const slashUsage = (out, pager, opts) => {
	const o = resolve(opts);
	const buf = [];
	const w = (s) => {
		buf.push(s);
	...
	if (o.useReadline) w("  \\s [FILE]              display history or save it to file\n");
	...
	w("\n");
	w("Connection\n");
	if (o.currentDb) w(`  \\c[onnect] {[DBNAME|- USER|- HOST|- PORT|-] | conninfo}
                         connect to new database (currently "${o.currentDb}")\n`);
	else w("  \\c[onnect] {[DBNAME|- USER|- HOST|- PORT|-] | conninfo}\n                         connect to new database (currently no connection)\n");
...
*/
const slashUsageHelp = (out, topic, opts) => {
	switch (topic) {
		case "commands":
...
const helpVariables = (out, opts) => {
	...
	w("Usage:\n");
	if (!o.win32) w(`  NAME=VALUE [NAME=VALUE] ${o.progname} ...\n  or \\setenv NAME [VALUE] inside psql\n\n`);
	else w(`  set NAME=VALUE\n  ${o.progname} ...\n  or \\setenv NAME [VALUE] inside psql\n\n`);
...
const writeCommandList = (out, screenWidth) => {
	...
	const colW = names.reduce((m, n) => Math.max(m, n.length), 0) + 2;
...
*/
const helpSQL = (out, topic, screenWidth) => {
...
//#endregion
export { helpSQL, helpVariables, setHelpPagerOpener, slashUsage, slashUsageHelp, usage };
