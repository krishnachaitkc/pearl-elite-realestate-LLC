...
const isApiKeyMethod = (method) => API_KEY_METHODS.includes(method);
/**
...
*/
const mintedKeyName = (profile, now = /* @__PURE__ */ new Date(), suffix = randomBytes(2).toString("hex")) => {
	return `neon-cli-${profile}-${now.toISOString().replace(/[-:]/g, "").replace(/\.\d+Z$/, "Z")}-${suffix}`;
};
/**
...
*/
const identityFromAuthDetails = (details, email) => {
	if (details.auth_method === "api_key_org") return { label: details.account_id };
	return {
...
*/
const notAnApiKeyMessage = (method) => `That credential authenticates as "${method}", not an API key. Create a key with \`neon api-keys create --name <name>\`, or have one minted with \`neon profile create <name> --mint\`.`;
//#endregion
export { identityFromAuthDetails, isApiKeyMethod, mintedKeyName, notAnApiKeyMessage };
