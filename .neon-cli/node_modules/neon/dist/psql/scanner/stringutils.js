//#region src/psql/scanner/stringutils.ts
/**
...
*/
const strtokx = (input, whitespace, delim, quote, escape, eAcceptInUnquoted, atEol, encoding) => {
	let i = 0;
	...
	if (delim.length > 0 && delim.includes(input[i])) {
		const token = input[i];
		i++;
		if (!atEol) while (i < n && whitespace.includes(input[i])) i++;
		return {
			token,
			rest: input.slice(i)
		};
	...
	if (eAcceptInUnquoted && p + 1 < n && eAcceptInUnquoted.includes(input[p]) && input[p + 1] === "'") {
		effectiveQuote = "'";
		effectiveEscape = "\\";
		p++;
	}
	if (effectiveQuote.length > 0 && effectiveQuote.includes(input[p])) {
		...
		if (!atEol) while (p < n && whitespace.includes(input[p])) p++;
		return {
			token,
			rest: input.slice(p)
		};
...
*/
const quoteIfNeeded = (value, escapeChars, quote) => {
	if (quote.length !== 1) throw new Error("quoteIfNeeded: quote must be exactly one character");
	let needsQuotes = false;
	let escaped = "";
	for (const c of value) if (c === quote) {
		needsQuotes = true;
		escaped += quote + quote;
	} else {
...
*/
const dequote = (value, quote) => {
	if (quote.length !== 1) throw new Error("dequote: quote must be exactly one character");
	if (value.length < 2 || !value.startsWith(quote) || !value.endsWith(quote)) return value;
	const inner = value.slice(1, -1);
	let out = "";
	let i = 0;
	while (i < inner.length) if (inner[i] === quote && inner[i + 1] === quote) {
		out += quote;
		i += 2;
	} else {
		out += inner[i];
...
const quoteSqlLiteral = (value) => {
	...
	let inner = "";
	for (const c of value) if (c === "'") inner += "''";
	else if (c === "\\") {
		inner += "\\\\";
		needsEscape = true;
	} else inner += c;
...
const quoteSqlIdent = (value) => {
	let inner = "";
	for (const c of value) inner += c === "\"" ? "\"\"" : c;
	return `"${inner}"`;
};
const VAR_NAME_CONT_RE = /[A-Za-z0-9_\x80-\xff]/;
const VAR_NAME_START_RE = /[A-Za-z_\x80-\xff]/;
const isVarNameStart = (c) => c !== void 0 && VAR_NAME_START_RE.test(c);
const isVarNameCont = (c) => c !== void 0 && VAR_NAME_CONT_RE.test(c);
/**
...
const tryConsumeVarSubstitution = (s, i, varLookup) => {
	if (varLookup === void 0) return null;
	if (s[i] !== ":") return null;
	const next = s[i + 1];
	...
	if (next === "{" && s[i + 2] === "?") {
		...
		if (j > i + 3 && s[j] === "}") {
			const value = varLookup(s.slice(i + 3, j));
			return {
				end: j + 1,
				text: value !== void 0 ? "TRUE" : "FALSE"
			};
		}
		return null;
	}
	if (next === "\"") {
		...
		while (j < s.length && isVarNameCont(s[j])) j++;
		if (j > i + 2 && s[j] === "\"") {
			const value = varLookup(s.slice(i + 2, j));
	...
	if (isVarNameStart(next)) {
		let j = i + 1;
		while (j < s.length && isVarNameCont(s[j])) j++;
		const value = varLookup(s.slice(i + 1, j));
		if (value === void 0) return {
			end: j,
			text: s.slice(i, j)
		};
...
//#endregion
export { dequote, quoteIfNeeded, quoteSqlIdent, quoteSqlLiteral, strtokx, tryConsumeVarSubstitution };
