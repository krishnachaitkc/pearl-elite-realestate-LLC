...
const hasFetchMethod = (value) => typeof value === "object" && value !== null && "fetch" in value && typeof value.fetch === "function";
/**
...
const hasUpgradeMethod = (value) => typeof value === "object" && value !== null && "upgrade" in value && typeof value.upgrade === "function";
/**
...
*/
const resolveUpgradeHandler = (mod) => {
	if (typeof mod.upgrade === "function") return mod.upgrade;
	const defaultExport = mod.default;
...
const isAddressInUse = (err) => typeof err === "object" && err !== null && err.code === "EADDRINUSE";
const DEFAULT_SEARCH_BASE = 8787;
const MAX_SEARCH_STEPS = 100;
const bindPort = async (server, selection, hostname) => {
	if (selection.mode === "explicit") return listen(server, selection.port, hostname);
	for (let step = 0; step < MAX_SEARCH_STEPS; step++) try {
...
*/
const startRuntime = async ({ source, port, hostname }) => {
...
*/
const portSelectionFromEnv = (env) => {
	const explicit = env.NEON_DEV_PORT;
	if (explicit !== void 0 && explicit !== "") return {
		mode: "explicit",
		port: parsePort(explicit, "NEON_DEV_PORT")
	};
	const injected = env.PORT;
	if (injected !== void 0 && injected !== "") return {
		mode: "explicit",
		port: parsePort(injected, "PORT")
	};
...
const parsePort = (value, varName) => {
	...
	if (!Number.isInteger(port) || port < 0 || port > 65535) throw new Error(`Invalid ${varName}: "${value}"`);
	return port;
};
...
if (isDirectExecution()) {
	...
	startRuntime({
		source,
		port: portSelectionFromEnv(process.env)
	}).catch((err) => {
...
//#endregion
export { portSelectionFromEnv, resolveFetchHandler, resolveUpgradeHandler, startRuntime, withErrorBoundary };
