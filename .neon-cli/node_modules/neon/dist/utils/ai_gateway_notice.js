...
//#region src/utils/ai_gateway_notice.ts
/**
...
*/
const FREE_SUBSCRIPTION_TYPES = /* @__PURE__ */ new Set(["free_v2", "free_v3"]);
/**
...
*/
const aiGatewayUpgradeUrl = (orgId) => orgId ? `https://console.neon.tech/app/${orgId}/billing` : "https://console.neon.tech/app/billing";
/**
...
*/
const aiGatewayModelsUrl = (projectId, branchId) => `https://console.neon.tech/app/projects/${projectId}/branches/${branchId}/ai-gateway`;
const isFreePlan = (subscriptionType) => subscriptionType !== void 0 && FREE_SUBSCRIPTION_TYPES.has(subscriptionType);
/**
...
*/
const hasFlagshipModels = (modelIds) => modelIds.some((id) => id.includes("opus") || id.includes("codex") || id.endsWith("-pro"));
/**
...
*/
const freePlanBlockMessage = (upgradeUrl) => `This neon.ts enables the AI Gateway, which isn't available on the Free plan — the gateway won't serve model requests. Upgrade to a paid plan and re-run, or remove \`preview.aiGateway\` from neon.ts. Upgrade here: ${upgradeUrl}`;
/**
...
*/
const buildAiGatewayNotice = ({ subscriptionType, modelIds, upgradeUrl, moreModelsUrl }) => {
	if (isFreePlan(subscriptionType)) return {
		level: "warning",
		message: `AI Gateway is enabled, but the gateway does not serve model requests on the Free plan. Upgrade to a paid plan to start making requests: ${upgradeUrl}`
	};
	if (modelIds !== void 0 && modelIds.length > 0 && !hasFlagshipModels(modelIds)) return {
		level: "warning",
		message: `AI Gateway is in public beta and not every model is enabled for your account yet, so some models are missing from the catalog. Request access to more models here: ${moreModelsUrl}`
	};
	return null;
};
...
*/
const extractEnabledModelIds = (body) => {
	if (!isRecord(body) || !Array.isArray(body.data)) return null;
	const ids = [];
	for (const entry of body.data) if (isRecord(entry) && typeof entry.id === "string" && entry.enabled === true) ids.push(entry.id);
	return ids;
};
/**
...
*/
const fetchGatewayModelIds = async (baseUrl, token) => {
	try {
		const res = await fetch(`${baseUrl.replace(/\/+$/, "")}/v1/models`, { headers: { Authorization: `Bearer ${token}` } });
...
*/
const assertAiGatewayProvisionable = async (params) => {
	let subscriptionType;
	let orgId;
	try {
		const { data } = await params.apiClient.getProject(params.projectId);
		subscriptionType = data.project.owner?.subscription_type;
		orgId = data.project.org_id ?? void 0;
	} catch {
		return;
	}
	if (isFreePlan(subscriptionType)) throw new Error(freePlanBlockMessage(aiGatewayUpgradeUrl(orgId)));
};
/**
...
*/
const warnAiGateway = async (params) => {
	try {
		const { data } = await params.apiClient.getProject(params.projectId);
		const subscriptionType = data.project.owner?.subscription_type;
		const orgId = data.project.org_id ?? void 0;
		const modelIds = params.gateway ? await fetchGatewayModelIds(params.gateway.baseUrl, params.gateway.token) ?? void 0 : void 0;
...
//#endregion
export { aiGatewayModelsUrl, aiGatewayUpgradeUrl, assertAiGatewayProvisionable, buildAiGatewayNotice, extractEnabledModelIds, fetchGatewayModelIds, freePlanBlockMessage, hasFlagshipModels, isFreePlan, warnAiGateway };
