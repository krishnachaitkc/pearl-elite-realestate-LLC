...
//#region src/psql/io/pgservice.ts
/**
...
*/
const defaultPgServiceFilePath = (env = process.env) => {
	const out = [];
	const explicit = env.PGSERVICEFILE;
	if (explicit !== void 0 && explicit.length > 0) out.push(explicit);
	if (isWindows) {
		const appdata = env.APPDATA;
		if (appdata !== void 0 && appdata.length > 0) out.push(path.join(appdata, "postgresql", ".pg_service.conf"));
	} else {
		const home = env.HOME ?? os.homedir();
		if (home.length > 0) out.push(path.join(home, ".pg_service.conf"));
	}
	const sysDir = env.PGSYSCONFDIR;
	if (sysDir !== void 0 && sysDir.length > 0) out.push(path.join(sysDir, "pg_service.conf"));
	if (!isWindows) out.push("/etc/pg_service.conf");
	return out;
};
/**
...
*/
const parsePgServiceContent = (content) => {
...
*/
const loadPgServices = async (paths, env = process.env) => {
	const candidates = paths ?? defaultPgServiceFilePath(env);
	const userSpecified = env.PGSERVICEFILE;
	const userSpecifiedAbs = userSpecified !== void 0 && userSpecified.length > 0 ? userSpecified : null;
	for (const p of candidates) {
		const content = await readIfExists(p);
		if (content === null) {
			if (userSpecifiedAbs !== null && p === userSpecifiedAbs) throw new Error(`service file "${p}" not found`);
			continue;
		}
...
*/
const lookupService = (services, name) => services.get(name);
//#endregion
export { defaultPgServiceFilePath, loadPgServices, lookupService, parsePgServiceContent };
