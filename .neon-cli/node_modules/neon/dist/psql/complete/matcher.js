//#region src/psql/complete/matcher.ts
/**
...
*/
const MatchAny = null;
const MatchAnyExcept = (pattern) => "!" + pattern;
...
*/
const wordMatches = (pattern, word, caseSensitive = false) => {
	if (pattern === null) return true;
	if (pattern.startsWith("!")) return !wordMatches(pattern.slice(1), word, caseSensitive);
	...
	for (;;) {
		...
		if (starIdx >= 0) {
			...
			if (wordlen >= beforeLen + afterLen && cimatch(word, cursor, beforeLen, caseSensitive) && cimatch(word.slice(wordlen - afterLen), cursor.slice(starIdx + 1), afterLen, caseSensitive)) return true;
		} else if (wordlen === i && cimatch(word, cursor, wordlen, caseSensitive)) return true;
...
*/
const Matches = (prev, patterns, caseSensitive = false) => {
	if (prev.length !== patterns.length) return false;
	for (let k = 0; k < patterns.length; k++) if (!wordMatches(patterns[k], prev[k], caseSensitive)) return false;
	return true;
};
/**
...
*/
const TailMatches = (prev, patterns, caseSensitive = false) => {
	if (prev.length < patterns.length) return false;
	const offset = prev.length - patterns.length;
	for (let k = 0; k < patterns.length; k++) if (!wordMatches(patterns[k], prev[offset + k], caseSensitive)) return false;
	return true;
};
/**
...
*/
const HeadMatches = (prev, patterns, caseSensitive = false) => {
	if (prev.length < patterns.length) return false;
	for (let k = 0; k < patterns.length; k++) if (!wordMatches(patterns[k], prev[k], caseSensitive)) return false;
	return true;
};
...
*/
const splitForCompletion = (input, cursor) => {
	...
	const inWhitespace = charBefore === " " || charBefore === "	" || charBefore === "\n" || charBefore === "\r";
	if (last.end === head.length && !inWhitespace) return {
...
//#endregion
export { HeadMatches, MatchAny, MatchAnyExcept, Matches, TailMatches, splitForCompletion, tokenize, wordMatches };
