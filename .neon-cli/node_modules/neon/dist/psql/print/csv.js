...
const csvPrinter = {
	...
	printQuery(rs, opts, out) {
		...
		const sep = topt.csvFieldSep !== void 0 && topt.csvFieldSep !== "" ? topt.csvFieldSep : ",";
		if (sep.length !== 1 || sep === "\"" || sep === "\n" || sep === "\r") throw new RangeError(`csv_fieldsep must be a single character other than '"', '\\n', or '\\r' (got ${JSON.stringify(sep)})`);
		...
		let outBuf = "";
		if (expanded) for (const row of cells) row.forEach((value, colIdx) => {
			outBuf += csvField(headers[colIdx], sep) + sep + csvField(value, sep) + "\n";
		});
		else {
			if (topt.startTable && !topt.tuplesOnly) outBuf += headers.map((h) => csvField(h, sep)).join(sep) + "\n";
			for (const row of cells) outBuf += row.map((c) => csvField(c, sep)).join(sep) + "\n";
		}
		out.write(outBuf);
...
const csvField = (value, sep) => {
	if (!(value.includes(sep) || value.includes("\"") || value.includes("\n") || value.includes("\r") || value === "\\." || sep === "\\" || sep === ".")) return value;
