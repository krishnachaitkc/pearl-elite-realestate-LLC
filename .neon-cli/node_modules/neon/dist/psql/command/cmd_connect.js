...
//#region src/psql/command/cmd_connect.ts
/**
...
const setCmdConnectDeps = (overrides) => {
...
const readConnectionOptions = (conn) => {
	if (!conn) return null;
	const raw = conn.opts;
	return raw !== void 0 && raw !== null && typeof raw === "object" ? raw : null;
};
const KEEP = "-";
/**
...
*/
const parseConnectArgs = (rawArgs) => {
	const trimmed = rawArgs.trim();
	if (trimmed.length === 0) return {};
	if (trimmed.startsWith("postgresql://") || trimmed.startsWith("postgres://")) return parseUri(trimmed);
	if (/^[A-Za-z_][A-Za-z0-9_]*=/.test(trimmed) || /\s[A-Za-z_][A-Za-z0-9_]*=/.test(trimmed)) return parseConninfo(trimmed);
	...
	if (tokens.length >= 4 && tokens[3] !== KEEP) {
		...
		if (!Number.isFinite(port) || port <= 0 || port > 65535) return { error: `invalid port number "${tokens[3]}"` };
		out.port = port;
	}
	return out;
};
...
const applyConnInfoKey = (out, key, value) => {
	switch (key) {
		...
		case "port": {
			...
			if (!Number.isFinite(port) || port <= 0 || port > 65535) return { error: `invalid port "${value}"` };
			out.port = port;
			break;
		}
		case "user":
		...
		case "sslmode":
			if (![
				...
				"verify-ca",
				"verify-full"
			].includes(value)) return { error: `invalid sslmode "${value}"` };
			out.ssl = value;
			break;
		case "channel_binding":
			if (![
				...
				"prefer",
				"require"
			].includes(value)) return { error: `invalid channel_binding "${value}"` };
		...
		case "connect_timeout": {
			...
			if (!Number.isFinite(n) || n < 0) return { error: `invalid connect_timeout "${value}"` };
			out.connectTimeoutMs = n * 1e3;
			break;
		}
		case "options": out.options = value;
	}
	return null;
};
const parseUri = (raw) => {
	...
	try {
		...
		if (url.port.length > 0) {
			...
			if (!Number.isFinite(port) || port <= 0 || port > 65535) return { error: `invalid port in URI` };
			out.port = port;
		}
		if (url.username.length > 0) out.user = decodeURIComponent(url.username);
		if (url.password.length > 0) out.password = decodeURIComponent(url.password);
...
const parseConninfo = (raw) => {
	...
	for (const pair of pairs) {
		...
		if (eq < 0) return { error: `missing "=" after "${pair}" in connection info` };
...
*/
const mergeConnectOpts = (settings, override, prior = null) => {
	const vars = settings.vars;
	const ov = {};
	for (const [k, v] of Object.entries(override)) if (v !== void 0) ov[k] = v;
	const seed = prior ? { ...prior } : {};
	const hostChanged = ov.host !== void 0 && ov.host !== prior?.host;
	if (hostChanged || ov.port !== void 0 && ov.port !== prior?.port || ov.user !== void 0 && ov.user !== prior?.user) delete seed.password;
	...
	const port = merged.port ?? (portStr !== void 0 ? parseInt(portStr, 10) : 5432);
	if (!Number.isFinite(port) || port <= 0 || port > 65535) return { error: `invalid port number` };
	const user = merged.user ?? vars.get("USER") ?? process.env.USER ?? process.env.USERNAME ?? "";
	if (user.length === 0) return { error: "no user name specified" };
...
const cmdConnect = {
	...
	run: async (ctx) => {
		...
		if (rawArgs.trim().length === 0) return runConninfo(ctx);
...
const pickActivePrinter = (settings) => {
	switch (settings.popt.topt.format) {
		case "aligned":
		case "wrapped": return alignedPrinter;
		case "unaligned": return unalignedPrinter;
		case "csv": return csvPrinter;
		case "json": return jsonPrinter;
		case "html": return htmlPrinter;
		case "asciidoc": return asciidocPrinter;
		case "latex": return latexPrinter;
		case "latex-longtable": return latexLongtablePrinter;
		case "troff-ms": return troffMsPrinter;
		default: return alignedPrinter;
	}
};
...
const buildConninfoRows = (ctx, db) => {
	...
	const database = ctx.settings.vars.get("DBNAME") ?? db.database ?? "";
	const user = ctx.settings.vars.get("USER") ?? db.user ?? "";
	const host = info?.host ?? ctx.settings.vars.get("HOST") ?? "";
	const hostaddr = info?.hostaddr ?? null;
	const port = info?.port ?? Number(ctx.settings.vars.get("PORT") ?? NaN);
	const portStr = Number.isFinite(port) ? String(port) : "";
	const rows = [];
	rows.push(["Database", database]);
	rows.push(["Client User", user]);
	if (host.startsWith("/")) {
		if (hostaddr !== null && hostaddr.length > 0) rows.push(["Host Address", hostaddr]);
		else rows.push(["Socket Directory", host]);
	} else {
		rows.push(["Host", host]);
		if (hostaddr !== null && hostaddr.length > 0 && hostaddr !== host) rows.push(["Host Address", hostaddr]);
	}
	...
	if (tls) {
		...
		rows.push(["SSL Key Bits", tls.keyBits !== null ? String(tls.keyBits) : ""]);
		rows.push(["SSL Cipher", tls.cipher]);
		rows.push(["SSL Compression", tls.compression !== "off" ? "true" : "false"]);
		rows.push(["ALPN", tls.alpn && tls.alpn.length > 0 ? tls.alpn : "none"]);
	}
	rows.push(["Superuser", ctx.settings.db?.parameterStatus("is_superuser") ?? "unknown"]);
	rows.push(["Hot Standby", ctx.settings.db?.parameterStatus("in_hot_standby") ?? "unknown"]);
	return rows;
};
...
*/
const scramSha256Verifier = (password, salt, iterations = 4096) => {
...
function defaultReadLine(prompt, opts) {
	return readLine(prompt, { echo: opts.echo });
}
...
const cmdPassword = {
	...
	run: async (ctx) => {
		...
		let user = ctx.nextArg("sql-id");
		if (user === null) {
			try {
				const row = (await db.query("SELECT CURRENT_USER")).rows[0] ?? [];
		...
		const sql = "ALTER USER " + db.escapeIdentifier(user) + " PASSWORD " + db.escapeLiteral(verifier);
		try {
...
*/
const registerConnectCommands = (registry) => {
...
//#endregion
export { cmdConnect, cmdConninfo, cmdEncoding, cmdPassword, mergeConnectOpts, parseConnectArgs, registerConnectCommands, scramSha256Verifier, setCmdConnectDeps };
