import { t as storeFor } from "./_chunks/credential_io-D8AR-jvB.js";
import { log } from "./log.js";
import pkg_default from "./pkg.js";
import { isNeonApiError, messageFromBody } from "./api.js";
import { authFailureMessage, getAuthContext, locationFromContext } from "./auth_context.js";
...
import { ensureAuth, isAccessTokenUsable, refreshStoredCredentials } from "./commands/auth.js";
...
function supersededOnDisk(rejectedToken) {
	if (rejectedToken === void 0) return false;
	...
	try {
		...
		return loaded !== null && loaded.credentials.access_token !== rejectedToken && isAccessTokenUsable(loaded.credentials, Date.now());
	} catch {
		return false;
	}
}
async function recoverFrom401(canRetry) {
	const context = getAuthContext();
	...
	if (supersededOnDisk(context.accessToken)) {
		log.debug("The rejected token has already been replaced on disk; retrying with the current one");
		return true;
	}
	const at = locationFromContext(context);
	if (at === null || context.oauthHost === void 0 || context.clientId === void 0) {
		log.error(authFailureMessage(context));
		return false;
	}
	try {
		if (await refreshStoredCredentials(at, {
			apiHost: "",
		...
		if (err instanceof AuthRefreshError) {
			...
			if (err.terminal) log.error(`Run \`neon auth --profile ${context.profile ?? "DEFAULT"}\` to sign in again.`);
			return false;
		}
		log.debug("Refresh after 401 failed: %s", err instanceof Error ? err.message : "unknown error");
	}
	log.error(authFailureMessage(context));
	return false;
}
async function handleError(msg, err, canRetry) {
	...
	if (err instanceof AuthRefreshError) {
		...
		if (err.terminal) {
			const context = getAuthContext();
			log.error(`Run \`neon auth --profile ${context?.profile ?? "DEFAULT"}\` to sign in again.`);
		}
...
(async () => {
	...
	while (attempts < MAX_ATTEMPTS) try {
		...
		attempts++;
		if (!await handleError("", err, attempts < MAX_ATTEMPTS) || attempts >= MAX_ATTEMPTS) {
			await closeAnalytics();
			process.exit(1);
		}
	}
