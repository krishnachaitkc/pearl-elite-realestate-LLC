...
import { spawnCliChild } from "../init/child.js";
import { agentSetupDoneLabel, formatInitDone, printInitBanner, printInitDone, shouldPrintInitBanner } from "../init/chrome.js";
import { coerceAgentFlag } from "../utils/agent_flag.js";
import { assertNamedAgentTooling, chooseYesAgentTooling, initPluginAgents, initSkillsMcpAgents, postScaffoldActions, projectContextFile, resolveNamedAgents } from "../init/plan.js";
...
const describe = "Scaffold a new project from a Neon starter template, then install agent tooling and link a Neon project";
...
}).example("$0 bootstrap my-app", "Create ./my-app from an interactively chosen template").example("$0 bootstrap . --template hono", "Scaffold the Hono template into the current directory").example("$0 bootstrap my-app --default", "Skip the pickers; link --yes still asks for a project unless one is already linked").example("$0 bootstrap --list-templates --output json", "Print the template catalog as JSON").example("$0 bootstrap my-app --agent cursor --agent claude-code", "Skip agent selection; install the plugin for those agents").epilogue(helpEpilogue("--agent / -a is forwarded to plugins, or to skills and mcp, not both. It skips agent selection, including with --default.", helpCsv("Plugin agents", initPluginAgents()), helpCsv("Skills and MCP agents", initSkillsMcpAgents()))).strict();
const handler = async (props) => {
	...
	if (shouldPrintInitBanner(props.default)) printInitBanner();
	const named = resolveNamedAgents(props.agent ?? []);
	if (props.agentSetup !== false) assertNamedAgentTooling(named, "bootstrap", {
		...props.directory !== void 0 && props.directory.length > 0 ? { directory: props.directory } : {},
...
*/
const resolveTemplateList = async (props) => props.template && findTemplate(FALLBACK_TEMPLATES, props.template) ? FALLBACK_TEMPLATES : fetchTemplates();
/**
...
const formatTemplateTitle = (template) => {
	if (!template.services || template.services.length === 0) return template.title;
...
const resolveSelectedTemplate = async (props, interactive, templates) => {
	if (props.template) {
		...
		if (!template) throw new Error(`Unknown template "${props.template}". Available templates: ${templateIds(templates)}.`);
		return template;
	}
	if (props.default) {
		const fallback = templates[0];
		if (!fallback) throw new Error("No templates available to scaffold from.");
		return fallback;
	}
	if (!interactive) throw new Error(`No template selected. Re-run in an interactive terminal to pick one, or pass --template <id>. Available templates: ${templateIds(templates)}.`);
...
const resolveTargetDir = async (props, interactive, template) => {
	...
	if (dir === void 0) {
		if (props.default) return resolve(process.cwd(), defaultDirName(template));
		if (!interactive) throw new Error(`No target directory given. Pass one, e.g. \`${getCliName()} bootstrap my-app\` (or "." for the current directory).`);
		const { value } = await prompts({
			...
			validate: (input) => input && input.trim().length > 0 ? true : "Enter a directory (use \".\" for the current directory)."
		});
...
const scaffold = async (template, targetDir) => {
	...
	const filesWritten = await scaffoldTemplate(template, targetDir, { onWarn: (message) => {
		log.warning(message);
	} });
	log.info("Scaffolded %d files into %s.", filesWritten, targetDir);
	return filesWritten;
};
const runPostScaffoldSteps = async (props, targetDir, interactive, template, named) => {
	...
	if (props.default) {
		await runDefaultSteps(props, targetDir, defaultPm, neonConfig, template, named);
		return;
	}
	if (!interactive) {
		printDoneSummary({
	...
	let pm = defaultPm;
	const wantInstall = props.install && await confirm(installPrompt(inferred));
	if (wantInstall) pm = inferred ?? await selectPackageManager();
	const wantGit = props.git && !isGitRepo(targetDir) && await confirm("Initialize a git repository?");
	const agentSetup = props.agentSetup === false ? "skip" : named.length > 0 ? chooseYesAgentTooling(named).setup : await (props.pickAgentSetup ?? pickAgentSetupInteractively)();
	const canLink = props.link && !(neonConfig && !wantInstall);
	if (props.link && !canLink) logSkippedLink(pm);
	const wantLink = canLink && await confirm(`Link this project to a Neon project now? (runs ${getCliName()} link)`);
...
const installPrompt = (inferred) => inferred ? `Install dependencies with ${inferred}?` : "Install dependencies?";
/** `link --yes` still asks for a project unless one is already linked. */
const runDefaultSteps = async (props, targetDir, pm, neonConfig, template, named) => {
	log.info("Quick start (--default): skipping the template, install, git, and agent pickers. link --yes still asks for a project unless one is already linked.");
	const wantGit = props.git && !isGitRepo(targetDir);
	const agentSetup = props.agentSetup === false ? "skip" : named.length > 0 ? chooseYesAgentTooling(named).setup : "skills-mcp";
	const outcome = await executePostScaffold(props, targetDir, {
		yes: true,
...
const executePostScaffold = async (props, targetDir, choices) => {
	...
	for (const action of actions) {
		...
		if (action === "agent") {
			agentSetup = await runAgentTooling({
				cwd: targetDir,
				yes: choices.yes,
				...choices.lockAgentSetup ? { agentSetup: choices.agentSetup } : {},
				...kids,
		...
		if (action === "install") {
			installed = await runCommand(choices.pm, installArgs(choices.pm), targetDir);
			if (!installed) installFailed = true;
			continue;
		}
		if (action !== "link") throw new Error(`Unhandled post-scaffold action: ${action}`);
...
const logSkippedLink = (pm) => {
	log.info(`Skipping the Neon link step: \`${getCliName()} link\` reads this project's neon.ts to pull env vars, which needs its dependencies. Run \`${formatInstallCommand(pm)}\`, then \`${getCliName()} link\`.`);
};
const bootstrapChildren = (props, targetDir) => {
...
*/
const initGitRepo = async (dir) => runCommand("git", ["init"], dir);
...
const selectPackageManager = async () => {
	...
	if (installed.length <= 1) return installed[0] ?? "npm";
...
const printScaffolded = (template, targetDir) => {
	log.info("Scaffolded \"%s\" into %s.", template.title, isCurrentDir(targetDir) ? "the current directory" : displayDir(targetDir));
};
const printDoneSummary = (input) => {
	...
	const deps = input.installFailed ? "install failed" : input.installed ? `installed with ${input.pm}` : "skipped";
	const git = input.gitFailed ? "init failed" : input.git ? "initialized" : "skipped";
	const project = input.linked ? "linked" : input.skippedLinkForDeps ? "skipped (needs dependencies)" : "not linked";
	const next = [];
	if (!isCurrentDir(input.targetDir)) next.push(`cd ${displayDir(input.targetDir)}`);
	if (!input.installed) next.push(formatInstallCommand(input.pm));
	if (input.suggestLink) next.push(`${getCliName()} link`);
	if (!unfinished) next.push("See the README to run it.");
	printInitDone(formatInitDone({
		heading,
		rows: [
			{
				label: "Template",
				value: input.template.title
			},
			{
				label: "Directory",
				value: displayDir(input.targetDir)
			},
			{
				label: "Dependencies",
				value: deps
...
const finishPostScaffold = (input) => {
	printDoneSummary(input);
	if (input.installFailed || input.gitFailed) throw new Error("Setup did not finish.");
};
...
//#endregion
export { builder, command, describe, handler, bootstrap_exports as t };
