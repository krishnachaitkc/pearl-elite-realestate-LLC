...
*/
const getHistory = () => inMemoryHistory.slice();
...
const decodeEntry = (line) => {
	...
	for (let i = 0; i < line.length; i++) {
		if (line.charCodeAt(i) === 92 && i + 1 < line.length) {
			const next = line[i + 1];
			if (next === "n") {
				out += "\n";
				i++;
...
const shouldIgnore = (entry, prev, histcontrol) => {
	...
	if (ignoreDups && prev !== void 0 && prev === entry) return true;
	return false;
};
/**
...
*/
const loadHistory = async (filePath) => {
	let raw;
	try {
...
*     under 1 MiB.
*/
const appendHistory = async (filePath, entry, histcontrol = "none") => {
	if (histcontrol === "ignoredups" || histcontrol === "ignoreboth" || histcontrol === "ignorespace") {
		let prev;
	...
	recordHistory(entry);
	await promises.appendFile(filePath, encodeEntry(entry) + "\n", "utf8");
};
/**
...
*/
const truncateHistory = async (filePath, maxLines) => {
	if (maxLines <= 0) {
		try {
	...
	const body = entries.slice(entries.length - maxLines).map(encodeEntry).join("\n") + "\n";
	const dir = path.dirname(filePath);
	const base = path.basename(filePath);
	const tmpPath = path.join(dir, `.${base}.${randomUUID()}.tmp`);
	await promises.writeFile(tmpPath, body, {
		encoding: "utf8",
		mode: 384
	});
	try {
...
* for tests.
*/
const defaultHistoryPath = (env = process.env) => {
	const explicit = env.PSQL_HISTORY;
	if (explicit !== void 0 && explicit.length > 0) return explicit;
	if (process.platform === "win32") {
		const appdata = env.APPDATA;
		if (appdata !== void 0 && appdata.length > 0) return path.join(appdata, "postgresql", "psql_history");
	}
...
*/
const resolveHistSize = (env = process.env) => {
	const raw = env.HISTSIZE;
	if (raw === void 0 || raw === "") return DEFAULT_HISTSIZE;
	const n = Number(raw);
	if (!Number.isInteger(n) || n < 0) return DEFAULT_HISTSIZE;
	return n;
};
//#endregion
export { appendHistory, clearHistory, defaultHistoryPath, getHistory, loadHistory, recordHistory, resolveHistSize, truncateHistory };
