...
*/
const pumpReadable = async (conn, readable, copyIn) => {
	let aborted = null;
	try {
		for await (const chunk of readable) {
			const buf = typeof chunk === "string" ? Buffer.from(chunk, "utf8") : Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
...
*/
const drainCopyOut = async (copyOut, writable) => {
	for await (const chunk of copyOut) await new Promise((resolve, reject) => {
		writable.write(chunk, (err) => {
			if (err !== null && err !== void 0) reject(err);
			else resolve();
		});
	});
};
...
//#endregion
export { drainCopyOut, pumpReadable };
